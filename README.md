...
served by NGINX and Passenger. The current version is 3.8.6-3.
...
Finally, exec into your Puppet Master and check for the existence of the file:

    $ sudo docker exec -it ultramaster bash
    # test -f /proof && cat /proof"
...
Next, we need to sign `dockerduck`'s Puppet Agent certificate on `ultramaster`. Exec into `ultramaster` and sign the cert:

    $ sudo docker exec ultramaster bash
